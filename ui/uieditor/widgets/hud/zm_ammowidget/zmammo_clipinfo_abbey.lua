...
CoD.ZmAmmo_ClipInfo.new = function ( menu, controller )
	...
	TotalAmmo:setTopBottom( true, false, 0, 48 )
	...
	--[[
	local GasGlow = LUI.UIImage.new()
	GasGlow:setLeftRight( false, false, -90, 80 )
	GasGlow:setTopBottom( false, false, -45, 40 )
	GasGlow:setRGB( 1, 0.54, 0.19 )
	GasGlow:setAlpha( 0.48 )
	GasGlow:setZRot( -4 )
	GasGlow:setImage( RegisterImage( "uie_t7_core_hud_mapwidget_panelglow" ) )
	GasGlow:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_add" ) )
	self:addElement( GasGlow )
	self.GasGlow = GasGlow
	--]]
	...
	GasPercent:setRFTMaterial( LUI.UIImage.GetCachedMaterial( "ui_add" ) )
	GasPercent:setAlignment( Enum.LUIAlignment.LUI_ALIGNMENT_CENTER )
	GasPercent:setAlignment( Enum.LUIAlignment.LUI_ALIGNMENT_TOP )
	GasPercent:subscribeToModel(Engine.GetModel(Engine.GetModelForController(InstanceRef), "weaponAmmo.heatLevel"), function ( model )
		local heatLevel = Engine.GetModelValue( model )
		if heatLevel and not (heatLevel == -1) then
			GasPercent:setText( Engine.Localize( heatLevel ) .. "%" )
		end
	end )
	GasPercent:subscribeToModel(Engine.GetModel(Engine.GetModelForController(InstanceRef), "weaponAmmo.overheatStatus"), function ( model )
		local overheatStatus = Engine.GetModelValue( model )
		if overheatStatus and not (overheatStatus == -1) then
			if overheatStatus == 0 then
				--GasGlow:setAlpha(0.48)
				GasPercent:setAlpha(1)
				GasPercent:setRGB( 1, 0.99, 0.93 )
			elseif overheatStatus == 1 then
				--GasGlow:setAlpha(0.48)
				GasPercent:setAlpha(1)
				GasPercent:setRGB( 1, 1, 0 )
			else
				--GasGlow:setAlpha(0)
				GasPercent:setAlpha(1)
				GasPercent:setRGB(1, 0, 0)
			end
		else
			--GasGlow:setAlpha(0)
			GasPercent:setAlpha(0)
		end
	end )
	self:addElement( GasPercent )
	self.GasPercent = GasPercent
	...
	self.clipsPerState = {
		DefaultState = {
			DefaultClip = function ()
				...
				--[[GasPercent:completeAnimation()
				self.GasPercent:setAlpha( 1 )
				self.clipFinished( GasPercent, {} )--]]
			end,
			WeaponDual = function ()
