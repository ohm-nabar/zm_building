...
function enable_side_step()
{
	...
	while(isdefined(self) && !isdefined(self.targetname))
	{
		wait(0.05);
	}
...
function zombie_get_side_step()
{
	...
	{
		if ( IS_TRUE( self.b_n_zombie_can_side_step ) && randomFloat( 1 ) < self.n_zombie_side_step_step_chance )
			return "step";
...
{
	if ( !IS_TRUE( self.b_n_zombie_can_side_step ) )
		return 0;
