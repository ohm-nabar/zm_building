...
function __main__()
{
	...
	level.abbey_box_generators = [];

    if(GetDvarString("ui_mapname") == "zm_building")
    {
        level.abbey_box_generators["chest_2"] = 1; // Spawn Room
        level.abbey_box_generators["chest_1"] = 2; // Staminarch
        level.abbey_box_generators["start_chest_1"] = 4; // Lion Room
        level.abbey_box_generators["start_chest_2"] = 3; // Downstairs Room
    }
    else
    {
        level.abbey_box_generators["chest_1"] = 1; // Bell Tower
        level.abbey_box_generators["chest_2"] = 1; // Radio Gallery
        level.abbey_box_generators["chest_3"] = 1; // Choir
        level.abbey_box_generators["chest_4"] = 1; // Basilica
        level.abbey_box_generators["start_chest_1"] = 1; // Airfield
        ...
        level.abbey_box_generators["start_chest_2"] = 1; // URM Labs
...
//	println( "***** LOOKING FOR:  " + self.chest_origin.target );
	...
	self.unitrigger_stub.script_noteworthy = self.script_noteworthy;
...
function trigger_visible_to_player(player)
{
	...
	generator = level.abbey_box_generators[self.script_noteworthy];
	power_flag = "power_on" + generator;
	if(! (level flag::exists(power_flag) && level flag::get(power_flag)))
	{
		visible = false;
	}
