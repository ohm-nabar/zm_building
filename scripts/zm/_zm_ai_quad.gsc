...
function global_spawn_func( b_force, str_targetname, v_origin, v_angles)
{
	...
	while ( true )
	{
		if ( !IS_TRUE( self.ignorespawninglimit ) ) //TU1 - self.ignorespawninglimit - only to be used when nothing else will suffice
		{
...
function quad_death_explo( v_origin, a_death_vars )
{
	...
	for ( i = 0; i < a_players.size; i++ )
	{
		if ( distance( v_origin, a_players[ i ].origin) <= a_death_vars[ "explo_radius_plr" ] )
		{
			...
			{
				b_is_immune = a_players[ i ] [ [level.quad_gas_immune_func ] ]();
			}
	...
	{
		zombie_dist_sq = DistanceSquared( v_origin, zombie.origin);
		radius_sq = a_death_vars[ "explo_radius_zomb" ] * a_death_vars[ "explo_radius_zomb" ];
		if ( zombie_dist_sq <= radius_sq )
		{
			zombie DoDamage(level.zombie_health, v_origin, self);
		}
	}
}
