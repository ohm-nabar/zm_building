...
#define FLASHLIGHT_TYPE				"automatic"			// Change this to change the type of flashlight, "normal" and "automatic" are the only values that work
#define FLASHLIGHT_ACTIVATE			"in_area"			// Change this to change how the flashlight can be activated, "normal" allows the player to activate it (with both types of flashlight) and "in_area" only activates in specified areas (only works with "automatic")
...
// -------- UV light
#define FLASHLIGHT_USE_UV			false				// Whether or not the flashlight will use the UV version
